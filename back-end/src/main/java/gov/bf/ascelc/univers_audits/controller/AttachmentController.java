...
public class AttachmentController {
    ...
    public ResponseEntity<?> uploadFiles(
        ...
        log.info("Upload {} fichier(s) pour dossier {}",
                files.size(), dossierId);
        ...
        try {
            ...
            for (MultipartFile file : files) {
                try {
                    ...
                    AttachmentType attType =
                            detectType(mime, originalName);

                    Attachment att = Attachment.builder()
                            .dossier(dossier)
                            // Champs nullable=false obligatoires
            ...
            log.error("Erreur creation dossier upload: {}",
                    e.getMessage(), e);
    ...
    public ResponseEntity<?> listFiles(
        ...
        List<Map<String, Object>> files =
                attachmentRepository
                        ...
                        .map(att -> {
                            String mime =
                                    att.getMimeType() != null
                                            ? att.getMimeType()
                                            : (att.getContentType() != null
                                            ? att.getContentType()
                                            : "");
                            String name =
                                    att.getOriginalName() != null
                                            ? att.getOriginalName()
                                            : att.getFileName();
                            long size =
                                    att.getFileSize() != null
                                            ? att.getFileSize()
                                            : (att.getFileSizeBytes() != null
    ...
    public ResponseEntity<Resource> download(
        ...
        try {
            Path filePath =
                    Paths.get(att.getFilePath());
            Resource resource =
                    new UrlResource(filePath.toUri());

            if (!resource.exists()) {
                log.warn("Fichier introuvable: {}",
                        att.getFilePath());
                return ResponseEntity.notFound().build();
            }
    ...
    private AttachmentType detectType(
